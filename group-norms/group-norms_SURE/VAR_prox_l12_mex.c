...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (mxIsDouble(prhs[0])){
        ...
        if (mxIsComplex(prhs[0])){
            ...
            VAR_prox_l12_cplx_double(Xr, Xi, G, Rb, La, V, I, J, N, L);
        }else{
            const double *X = (double*) mxGetData(prhs[0]);
            VAR_prox_l12_real_double(X, G, Rb, La, V, I, J, N, L);
        }
    }else{
        ...
        if (mxIsComplex(prhs[0])){
            ...
            VAR_prox_l12_cplx_single(Xr, Xi, G, Rb, La, V, I, J, N, L);
        }else{
            const float *X = (float*) mxGetData(prhs[0]);
            VAR_prox_l12_real_single(X, G, Rb, La, V, I, J, N, L);
        }
    }
}
