...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (mxIsDouble(prhs[0])){
        ...
        if (mxIsComplex(prhs[0])){
            ...
            *norm = norm_d1inf_cplx_double(Xr, Xi, G, La, I);
        }else{
            const double *X = (double*) mxGetData(prhs[0]);
            *norm = norm_d1inf_real_double(X, G, La, I);
        }
    }else{
        ...
        if (mxIsComplex(prhs[0])){
            ...
            *norm = norm_d1inf_cplx_single(Xr, Xi, G, La, I);
        }else{
            const float *X = (float*) mxGetData(prhs[0]);
            *norm = norm_d1inf_real_single(X, G, La, I);
        }
    }
    mxFree(G);
}
