/*==================================================================
 * Y = prox_l12_d12_mex(X, G, Thrl, Thrd);
 *
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (mxIsDouble(prhs[0])){
        const double *Thrl = (double*) mxGetData(prhs[2]);
        const double *Thrd = (double*) mxGetData(prhs[3]);
        plhs[0] = mxDuplicateArray(prhs[0]);
        if (mxIsComplex(plhs[0])){
            double *Yr = (double*) mxGetData(plhs[0]);
            double *Yi = (double*) mxGetImagData(plhs[0]);
            prox_l12_d12_cplx_double(Yr, Yi, G, Thrl, Thrd, I);
        }else{
            double *Y = (double*) mxGetData(plhs[0]);
            prox_l12_d12_real_double(Y, G, Thrl, Thrd, I);
        }
    }else{
        const float *Thrl = (float*) mxGetData(prhs[2]);
        const float *Thrd = (float*) mxGetData(prhs[3]);
        plhs[0] = mxDuplicateArray(prhs[0]);
        if (mxIsComplex(plhs[0])){
            float *Yr = (float*) mxGetData(plhs[0]);
            float *Yi = (float*) mxGetImagData(plhs[0]);
            prox_l12_d12_cplx_single(Yr, Yi, G, Thrl, Thrd, I);
        }else{
            float *Y = (float*) mxGetData(plhs[0]);
            prox_l12_d12_real_single(Y, G, Thrl, Thrd, I);
        }
    }
    mxFree(G);
}
