...
void proj_d12_real_double(double *X, const int **G, const double *Bnd, const int I)
{
	int i, j, g, idx, idg;
	const int *gidx;
	double bnd, ng2, avg, shrink; 
    #pragma omp parallel for private(i, j, g, idx, idg, gidx, bnd, ng2, avg, shrink)
...
void proj_d12_cplx_double(double *Xr, double *Xi, const int **G, const double *Bnd, const int I)
{
	int i, j, g, idx, idg;
	const int *gidx;
	double bnd, ng2, avgr, avgi, shrink; 
    #pragma omp parallel for private(i, j, g, idx, idg, gidx, bnd, ng2, avgr, avgi, shrink)
    /* iterate over all groups */
    for (g=0; g<G[0][0]; g++){
        gidx = G[g+1];
        /* iterate over all frames */
...
void proj_d12_cplx_single(float *Xr, float *Xi, const int **G, const float *Bnd, const int I)
{
	int i, j, g, idx, idg;
	const int *gidx;
	float bnd, ng2, avgr, avgi, shrink; 
    #pragma omp parallel for private(i, j, g, idx, idg, gidx, bnd, ng2, avgr, avgi, shrink)
    /* iterate over all groups */
    for (g=0; g<G[0][0]; g++){
        gidx = G[g+1];
