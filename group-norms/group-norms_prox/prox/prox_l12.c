...
void prox_l12_real_double(double *X, const int **G, const double *Thr, const int I)
{
	int i, j, g, idg;
	const int *gidx;
	double x, thr, ng2, shrink; 
    #pragma omp parallel for private(i, j, g, idg, gidx, x, thr, ng2, shrink)
...
void prox_l12_cplx_double(double *Xr, double *Xi, const int **G, const double *Thr, const int I)
{
	int i, j, g, idx, idg;
	const int *gidx;
	double thr, ng2, shrink; 
    #pragma omp parallel for private(i, j, g, idx, idg, gidx, thr, ng2, shrink)
    /* iterate over all groups */
    for (g=0; g<G[0][0]; g++){
        gidx = G[g+1];
        /* iterate over all frames */
