...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (mxIsDouble(prhs[0])){
        ...
        if (mxIsComplex(plhs[0])){
            ...
            proj_l1inf_cplx_double(Yr, Yi, G, Bnd, I);
        }else{
            double *Y = (double*) mxGetData(plhs[0]);
            proj_l1inf_real_double(Y, G, Bnd, I);
        }
    }else{
        ...
        if (mxIsComplex(plhs[0])){
            ...
            proj_l1inf_cplx_single(Yr, Yi, G, Bnd, I);
        }else{
            float *Y = (float*) mxGetData(plhs[0]);
            proj_l1inf_real_single(Y, G, Bnd, I);
        }
    }
    mxFree(G);
}
