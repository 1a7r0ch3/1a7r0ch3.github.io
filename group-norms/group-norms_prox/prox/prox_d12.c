...
void prox_d12_real_double(double *X, const int **G, const double *Thr, const int I)
{
	int i, j, g, idx, idg;
	const int *gidx;
	double thr, ng2, avg, shrink; 
    #pragma omp parallel for private(i, j, g, idx, idg, gidx, thr, ng2, avg, shrink)
...
void prox_d12_cplx_double(double *Xr, double *Xi, const int **G, const double *Thr, const int I)
{
	int i, j, g, idx, idg;
	const int *gidx;
	double thr, ng2, avgr, avgi, shrink; 
    #pragma omp parallel for private(i, j, g, idx, idg, gidx, thr, ng2, avgr, avgi, shrink)
    /* iterate over all groups */
    for (g=0; g<G[0][0]; g++){
        gidx = G[g+1];
        /* iterate over all frames */
...
void prox_d12_cplx_single(float *Xr, float *Xi, const int **G, const float *Thr, const int I)
{
	int i, j, g, idx, idg;
	const int *gidx;
	float thr, ng2, avgr, avgi, shrink; 
    #pragma omp parallel for private(i, j, g, idx, idg, gidx, thr, ng2, avgr, avgi, shrink)
    /* iterate over all groups */
    for (g=0; g<G[0][0]; g++){
        gidx = G[g+1];
